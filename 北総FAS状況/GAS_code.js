...
//   - 結果が負になる場合は 0 にクランプ
// ★GASエディタから手動実行する。下の monthPrefix を当月の日付プレフィックスに書き換えてから実行
function recomputeMonthFromLogs() {
  var monthPrefix = "2026/05/"; // ★毎月変更: 当月のログ日付プレフィックス（末尾スラッシュ必須）
  var ss = SpreadsheetApp.openById(SHEET_ID);
  var src = ss.getSheetByName(SHEET_NAME);
  var logSheet = ss.getSheetByName("変更ログ");
